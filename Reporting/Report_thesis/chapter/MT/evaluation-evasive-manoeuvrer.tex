...
\begin{figure}[!p]
	\centering
	\includegraphics[width=.7\textwidth]{distance-passing-distance-advance.png} 
...
\todo{explain in more detail what causes the straight front showing max CPA}
...
\todo{Table with results of use case}
\todo{Change name in fig 7.5 from Tanker --> Gulf Valour}
\todo{Show plots of use case trough time, similar to a video}
...
\todo{Update axis name to distance till initial CPA; plots in same figure or axis should be equal (figure 7.1)}
\todo{Add help lines which show how I read the graph}
